...
func TestBuild_IncludesToolsAndUserSystem(t *testing.T) {
	...
	reg := tools.Default("/tmp/w", nil)
	s := Build(Params{Cfg: cfg, Reg: reg, Mode: ModeAgent, UserSystem: "custom", RepoInstructions: ""})
...
func TestBuild_RepoInstructions(t *testing.T) {
	...
	s := Build(Params{Cfg: cfg, Reg: reg, Mode: ModeAgent, RepoInstructions: "Use tabs."})
...
func TestBuild_ModePlan_IncludesPlanSection(t *testing.T) {
	cfg := &config.Config{Workspace: "/tmp/w"}
	reg := tools.DefaultReadOnlyPlan("/tmp/w")
	...
	if !strings.Contains(s, "Required written plan") || !strings.Contains(s, "does not remember past runs") {
		t.Fatalf("missing required plan guidance: %s", s)
	}
...
func TestBuild_ModeAsk_ReadOnlySections(t *testing.T) {
	cfg := &config.Config{Workspace: "/tmp/w"}
	reg := tools.DefaultReadOnly("/tmp/w")
